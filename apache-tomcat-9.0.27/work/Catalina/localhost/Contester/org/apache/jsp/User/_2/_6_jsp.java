 ...
 * Generated at: 2021-03-25 13:50:53 UTC
...
public final class _6_jsp extends org.apache.jasper.runtime.HttpJspBase
  ...
  public void _jspService(final javax.servlet.http.HttpServletRequest request, final javax.servlet.http.HttpServletResponse response)
    ...
    try {
      ...
      out.write("\t\t\t\t<center><h2>B. Шифровка сдвигом</h2></center>\n");
      out.write("\t\t\t\t<center>(Время: 1 сек. Память: 256 Мб)</center>\n");
      out.write("\t\t\t\t<h3>Условие:</h3>\n");
      out.write("\t\t\t\t<p>Петя и Вася придумали систему шифровки для обмена записками. Суть ее заключается в следующем: дана исходная строка \\( S \\). \\( S^{\\prime} \\) — циклический сдвиг строки влево (первый символ становится последним, а остальные перемещаются на одну позицию влево). \\( S^{\\prime \\prime} \\) — циклический сдвиг строки \\( S^{\\prime}\\) и т.д. Петя с Васей выписывают на листок бесконечную последовательность символов \\( SS^{\\prime}S^{\\prime \\prime}S^{\\prime \\prime \\prime} \\dotso \\). Если им необходимо зашифровать символ \\(C\\), то они ищут какое-либо вхождение этого символа в выписанную последовательность и записывают его порядковый номер \\(k\\). Нумерацию символов они ведут с единицы.</p>\n");
      out.write("\t\t\t\t<p>Злоумышленник Коля перехватил сообщение и выкрал исходную строку \\(S\\). Однако он не может определить, какой символ стоит в последовательности \\( SS^{\\prime}S^{\\prime \\prime}S^{\\prime \\prime \\prime} \\dotso \\) на \\(k\\)-ом месте. Помогите злоумышленнику Коле узнать, какой символ соответствует числу \\(k\\).</p>\n");
      out.write("\t\t\t\t<h3>Входные данные:</h3>\n");
      out.write("\t\t\t\t<p>Первая строка входного файла содержит строку, состоящую только из строчных латинских букв. Длина строки не превышает 100000 символов. Вторая строка входного файла содержит единственное целое число \\( 1 \\leq k \\leq 2 \\cdot 10^9 \\).</p>\n");
      out.write("\t\t\t\t<h3>Выходные данные:</h3>\n");
      out.write("\t\t\t\t<p>Единственная строка выходного файла должна содержать символ, который окажется на \\( k \\)-ом месте сформированной строки.</p>\n");
      ...
      out.write("\t\t\t\t\t");
 out.print("<form action=\"../run?contest=2&task=6&name="); 
